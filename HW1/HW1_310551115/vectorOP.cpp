...
float arraySumVector(float *values, int N)
{
  ...
  // iterate _pp_hadd_float => one partial sum
  // accumulate partial sums
  float sum = 0.0;
  __pp_vec_float partials, tmp, zeros;
  __pp_mask maskAll;
  maskAll = _pp_init_ones();
  _pp_vset_float(zeros, 0.0, maskAll);

  for (int i = 0; i < N; i += VECTOR_WIDTH)
  {
    int count = VECTOR_WIDTH >> 1;
    _pp_vload_float(partials, values + i, maskAll);
    while (count)
    {
      _pp_hadd_float(partials, partials);
      _pp_vadd_float(tmp, partials, zeros, maskAll);
      _pp_interleave_float(partials, tmp);
      count >>= 1;
    }
    sum += partials.value[0];
  }
