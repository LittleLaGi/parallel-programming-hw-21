...
__global__ void mandelKernel(float lowerX, float lowerY, float stepX, float stepY, int maxIterations,
    ...
    {   
        int Col = (blockIdx.x * blockDim.x + threadIdx.x) * group_size + gi;
...
{   
    ...
    int* __restrict__ M = (int*)malloc (resY * pitch);

    // image: 1600 x 1200
    // limit: (64, 16)
    dim3 dimBlock(16, 16);
    const int group_size = 4;
    dim3 dimGrid(resX / dimBlock.x / group_size, resY / dimBlock.y);

    mandelKernel<<<dimGrid, dimBlock>>>(lowerX, lowerY, stepX, stepY, maxIterations, Md, pitch / sizeof(int), group_size);
